...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
	...
	if(compare(getDynArr(heap, i), getDynArr(heap, j)) == -1)
		return i;
	else if((*compare)(getDynArr(heap, i), getDynArr(heap, j)) == 1)
		return j;
	else
		return j;
}
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
	...
	_adjustHeap(heap, sizeDynArr(heap) - 1, (sizeDynArr(heap)-2)/2, compare);
}
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
	int temp;
	int leftldx = pos*2+1;
	int rightldx = pos*2+2;

	if(rightldx < max){
		temp = _smallerIndexHeap(heap, leftldx, rightldx, compare);
		if((*compare)(getDynArr(heap, temp), getDynArr(heap, pos)) == -1)
			swapDynArr(heap, temp, pos);
		_adjustHeap(heap, max, temp, compare);
	}
	else if(leftldx < max){
		if((*compare)(getDynArr(heap, leftldx), getDynArr(heap, pos)) == -1)
			swapDynArr(heap, leftldx, pos);
	}
	else
		return;
}
...
void sortHeap(DynArr *heap, comparator compare)
{
	...
	int last = (heap->size)-1; // minus 1 to avoid going out of the bounds of the array
	int i;	
