...
{ 
	...
	return tree->cnt;    /*  returns the size*/
}
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	struct Node *newnode;
	if(cur == NULL){ /* assigns val to root if tree is empty */
		newnode = malloc(sizeof(struct Node));
		assert(newnode != NULL);
		newnode->val = val; 
		newnode->left = newnode->right = NULL; /* assigns left and right branches of new node */
		return newnode;
	}
...
void removeBSTree(struct BSTree *tree, TYPE val)
{
	...
	assert(tree != NULL && val != NULL);
	assert(containsBSTree(tree,val) == 1); 
...
void testAddNode() {
    ...
    printf("\nThe tree is has a size of: %d\n", sizeBSTree(tree));

    printf("\nThe tree looks like: ");
    printTree(tree); /* Prints the whole tree using printNode */

    printf("\nThe left most node is: ");
    print_type(_leftMost(tree->root)); /* Testing the left most node and print_type function */
	//tree->root = _removeLeftMost(tree->root->left);
	printTree(tree);
	}
...
void testCompareLetters(){
	printf("\nNow testing compareLetters!\n");
    ...
    addBSTree(tree, &myData1);
    //check the root node
	if (compareLetters(tree->root->val,&myData1) != 0) {
        printf("addNode() test: FAIL to insert john as root\n");
        return;
    }
	//check the tree->cnt value after adding a node to the tree
    else if (tree->cnt != 1) {
        printf("addNode() test: FAIL to increase count when inserting john as root\n");
        return;
    }
    else printf("addNode() test: PASS when adding john as root\n");
	...
	//check the position of the second element that is added to the BST tree
    if (compareLetters(tree->root->left->val,  &myData2) != 0) {
        printf("addNode() test: FAIL to insert alex as left child of root\n");
        return;
    }
    else if (tree->cnt != 2) {
        printf("addNode() test: FAIL to increase count when inserting alex as left of root\n");
        return;
    }
    else printf("addNode() test: PASS when adding alex as left of root\n");
}
